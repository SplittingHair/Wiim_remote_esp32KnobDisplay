...
void lcd_lvgl_Init(void)
{
  ...
  assert(lvgl_mux);
  xTaskCreatePinnedToCore(example_lvgl_port_task, "LVGL", EXAMPLE_LVGL_TASK_STACK_SIZE, NULL, 4, NULL, 1);
...
// AMOLED brightness via SH8601 DCS command 0x51 (0..255).
// Serialized with LVGL flushes via the lvgl lock.
void lcd_set_brightness(uint8_t level)
{
  if (amoled_panel_io_handle == NULL) return;
  if (example_lvgl_lock(200)) {
    uint8_t d = level;
    esp_lcd_panel_io_tx_param(amoled_panel_io_handle, 0x51, &d, 1);
    example_lvgl_unlock();
  }
}

// True AMOLED panel blank via the SH8601 PANEL DRIVER, which applies
// the QSPI command framing this panel requires. Raw tx_param does not.
void lcd_panel_sleep(bool sleep_on)
{
  lv_disp_t *disp = lv_disp_get_default();
  if (disp == NULL || disp->driver == NULL || disp->driver->user_data == NULL) return;
  esp_lcd_panel_handle_t panel = (esp_lcd_panel_handle_t)disp->driver->user_data;
  // SH8601 QSPI command framing: (0x02 << 24) | (cmd << 8) -- same envelope
  // the working disp_on_off uses. disp_sleep is unimplemented in this
  // component, so we send the framed sleep commands ourselves.
  #define SH8601_FRAMED_CMD(c) ((0x02UL << 24) | ((uint32_t)(c) << 8))
  // NOTE: must be called from the LVGL context (lv_async_call).
  // No locking here: the LVGL task already owns the display.
  if (sleep_on) {
    esp_lcd_panel_disp_on_off(panel, false);   // display off (0x28)
    esp_lcd_panel_io_tx_param(amoled_panel_io_handle,
                              SH8601_FRAMED_CMD(0x10), NULL, 0);  // sleep-in
  } else {
    esp_lcd_panel_io_tx_param(amoled_panel_io_handle,
                              SH8601_FRAMED_CMD(0x11), NULL, 0);  // sleep-out
    vTaskDelay(pdMS_TO_TICKS(120));            // panel spec wake time
    esp_lcd_panel_disp_on_off(panel, true);    // display on (0x29)
  }
}
